(() => {
  ...
  const entitlement=Object.freeze({
    ...
    async refresh(){if(!adapter?.currentEntitlement){emit({status:"unavailable",isPro:false,product:null,error:null});return state}emit({status:"loading",error:null});try{const [current,product]=await Promise.all([adapter.currentEntitlement(),adapter.product?.()]);emit({status:"ready",isPro:verified(current),product:product||null,error:null})}catch(error){emit({status:"error",isPro:false,product:null,error:String(error?.message||error)})}return state},
  ...
  overlay.innerHTML=`<header class="cue-pro-header-v1"><button class="cue-pro-back-v1" type="button" aria-label="戻る">‹</button><h1>CueScore Pro</h1><span></span></header><main class="cue-pro-scroll-v1"><section class="cue-pro-hero-v1"><div class="cue-pro-mark-v1"><img src="src/assets/logo/CueScore_LogoMark_White.svg" alt="" aria-hidden="true"></div><h2>CueScore Pro</h2><p class="cue-pro-lead-v1">記録をもっと残す。<br>プレーをもっと振り返る。</p></section><section class="cue-pro-values-v1"><div><i>✓</i><span>履歴無制限</span></div><div><i>✓</i><span>自己ベスト</span></div><div><i>✓</i><span>詳細分析・推移</span></div><div><i>✓</i><span>対戦相手別の振り返り</span></div><div><i>✓</i><span>Backup / Restore</span></div></section><p class="cue-pro-price-v1" data-pro-price>価格を取得できません</p><p class="cue-pro-once-v1">一度の購入でずっと利用できます</p><button class="cue-pro-buy-v1" type="button" data-pro-buy disabled>Proを購入</button><button class="cue-pro-restore-v1" type="button" data-pro-restore>購入を復元</button><p class="cue-pro-status-v1" data-pro-status></p><p class="cue-pro-legal-v1">購入機能はApp Storeの商品設定完了後に利用できます。保存済みデータは購入の有無にかかわらず削除されません。</p></main>`;
  document.body.appendChild(overlay);
  let returnFocus=null,currentSource="",replay=null,bypass=false;
  const status=overlay.querySelector("[data-pro-status]");
  function close(unlocked=false){overlay.hidden=true;document.body.classList.remove("cue-pro-open-v1");if(unlocked&&replay){const action=replay;replay=null;queueMicrotask(action)}else returnFocus?.focus?.();currentSource=""}
  ...
  function open(source,options={}){currentSource=sourceNames[source]?source:"analysis";returnFocus=options.trigger||document.activeElement;replay=typeof options.replay==="function"?options.replay:null;status.textContent=`${sourceNames[currentSource]}はProで利用できます。`;status.classList.remove("is-error");overlay.hidden=false;document.body.classList.add("cue-pro-open-v1");syncPaywall();overlay.querySelector(".cue-pro-back-v1")?.focus()}

(() => {
  ...
  const diagnostic=Object.freeze({
    ...
    planViewState(snapshot){if(snapshot?.isPro===true)return{status:"pro",label:"CueScore Pro ✓",action:"購入・復元について"};if(snapshot?.status==="ready")return{status:"free",label:"CueScore Free",action:"Proを購入・購入を復元"};return{status:"unknown",label:"確認中",action:"購入情報を確認中"}},
    freshProductStart(snapshot){return{...snapshot,status:"loading",product:null,error:null}},
    productViewState(snapshot,operationInFlight=false){if(snapshot?.status==="loading"&&!snapshot?.product)return{text:"価格を確認中…",canPurchase:false};if(snapshot?.status==="ready"&&snapshot?.product?.localizedPrice)return{text:String(snapshot.product.localizedPrice),canPurchase:!operationInFlight};return{text:"価格を取得できません",canPurchase:false}}
  });
  ...
  const sourceNames=Object.freeze({personalBest:"自己ベスト",analysis:"分析",opponents:"対戦相手別",historyLimit:"全履歴",backup:"バックアップ",restore:"データ復元",currentPlan:"プラン"});
  ...
  let adapter=null,state=Object.freeze({status:"unavailable",isPro:false,product:null,error:null,diagnostic:null}),refreshSerial=0;
  ...
  const entitlement=Object.freeze({
    ...
    beginFreshProductLoad(){if(!adapter?.product)return state;refreshSerial+=1;emit(diagnostic.freshProductStart(state));return state},
    async refresh(options={}){const freshProduct=options?.freshProduct===true,requestId=++refreshSerial;recordPurchase("ENTITLEMENT_REFRESH_STARTED");if(!adapter?.currentEntitlement){emit({status:"unavailable",isPro:false,product:null,error:null,diagnostic:isNative?{state:"BRIDGE_ERROR"}:null});recordPurchase("ENTITLEMENT_REFRESH_RESULT / unavailable");return state}const previouslyVerified=state.isPro===true,previousProduct=state.product;emit({status:"loading",product:freshProduct?null:previousProduct,error:null});let current;try{current=await adapter.currentEntitlement()}catch(error){if(requestId!==refreshSerial)return state;const safe=diagnostic.safeError(error);emit({status:previouslyVerified?"ready":"error",isPro:previouslyVerified,product:freshProduct?null:previousProduct,error:safe.classification,diagnostic:{state:"BRIDGE_ERROR"}});recordSafeError("ENTITLEMENT_REFRESH_REJECTED",error);return state}let product=null,productError=null,productDiagnostic=null;try{product=await adapter.product?.();if(product)productDiagnostic=diagnostic.fromProduct(product)}catch(error){productError=diagnostic.safeError(error).classification;productDiagnostic=diagnostic.fromError(error);recordSafeError("PRODUCT_REFRESH_REJECTED",error)}if(requestId!==refreshSerial)return state;const isPro=verified(current);emit({status:product||isPro?"ready":"error",isPro,product:product||(freshProduct?null:previousProduct)||null,error:productError,diagnostic:productDiagnostic});recordPurchase(`ENTITLEMENT_REFRESH_RESULT / entitlement=${yesNo(isPro)}`);return state},
  ...
  const refreshSafely=(phase,options)=>diagnostic.settleSafely(()=>entitlement.refresh(options),error=>recordSafeError(`ENTITLEMENT_REFRESH_BOUNDARY_${phase}`,error),entitlement.snapshot);
  ...
  function syncPaywall(){const s=entitlement.snapshot,productView=diagnostic.productViewState(s,operationInFlight);overlay.querySelector("[data-pro-price]").textContent=productView.text;overlay.querySelector("[data-pro-active]").hidden=!s.isPro;buyButton.hidden=s.isPro;buyButton.disabled=!productView.canPurchase;restoreButton.disabled=operationInFlight;if(s.isPro&&!operationInFlight&& !overlay.hidden){status.textContent="✓ CueScore Pro 有効";status.classList.remove("is-error");status.classList.add("is-success")}}
  function syncSettingsPlan(snapshot=entitlement.snapshot){const view=diagnostic.planViewState(snapshot);document.querySelectorAll("[data-settings-plan-state]").forEach(node=>{node.textContent=view.label;node.classList.toggle("is-pro",view.status==="pro");node.classList.toggle("is-unknown",view.status==="unknown")});document.querySelectorAll("[data-settings-plan-action]").forEach(button=>{button.dataset.planState=view.status;button.disabled=view.status==="unknown";const label=button.querySelector("[data-settings-plan-action-label]");if(label)label.textContent=view.action})}
  function open(source,options={}){currentSource=sourceNames[source]?source:"analysis";returnFocus=options.trigger||document.activeElement;replay=typeof options.replay==="function"?options.replay:null;originScroll=captureScroll(currentSource);status.textContent=`${sourceNames[currentSource]}はProで利用できます。`;status.classList.remove("is-error","is-success");entitlement.beginFreshProductLoad();overlay.hidden=false;document.body.classList.add("cue-pro-open-v1");syncPaywall();void refreshSafely("PRO_OPEN",{freshProduct:true});overlay.querySelector(".cue-pro-back-v1")?.focus({preventScroll:true})}
  ...
  entitlement.subscribe(snapshot=>{syncPaywall();syncSettingsPlan(snapshot);decorate();if(snapshot.isPro&&!lastDecoratedPro)window.renderMatchRecords?.();lastDecoratedPro=snapshot.isPro});
  ...
  document.addEventListener("click",event=>{const target=event.target;
    if(target.closest?.("[data-settings-plan-action]")){event.preventDefault();event.stopImmediatePropagation();const trigger=target.closest("button");if(diagnostic.planViewState(entitlement.snapshot).status!=="unknown")open("currentPlan",{trigger});return}
  ...
  new MutationObserver(()=>requestAnimationFrame(decorate)).observe(document.body,{subtree:true,childList:true});syncSettingsPlan();decorate();

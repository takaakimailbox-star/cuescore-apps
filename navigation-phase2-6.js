(() => {
  ...
  const matchRow=(record,player)=>{const s=side(record,player),o=opponent(record,s),isWin=won(record,s);return `<button type="button" class="hub-match-row-v2 match-card-c-v34 match-card-c-recent-v34" data-hub-match="${esc(record.id)}"><span class="match-card-c-main-v34"><small class="match-card-c-date-v34">${dateText(record)}</small><strong class="match-card-c-opponent-v34">${esc(o.name||"対戦相手")}</strong></span><span class="match-card-c-result-v34 ${isWin?"is-win":"is-loss"}">${isWin?"勝ち":"負け"}</span><span class="match-card-c-score-v34">${score(record,s)}<i>−</i>${score(record,s===1?2:1)}</span><b aria-hidden="true">›</b></button>`;};

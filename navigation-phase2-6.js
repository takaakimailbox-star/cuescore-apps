(() => {
  ...
  const matchRow=(record,player)=>{const s=side(record,player),o=opponent(record,s),isWin=won(record,s),cardClass=window.CueScoreMatchCardC?.classes?.("recent")||"match-card-c-v36 match-card-c-recent-v36";return `<button type="button" class="hub-match-row-v2 ${cardClass}" data-hub-match="${esc(record.id)}"><span class="match-card-c-main-v36"><small class="match-card-c-date-v36">${dateText(record)}</small><strong class="match-card-c-opponent-v36">${esc(o.name||"対戦相手")}</strong></span><span class="match-card-c-result-v36 ${isWin?"is-win":"is-loss"}">${isWin?"勝ち":"負け"}</span><span class="match-card-c-score-v36">${score(record,s)}<i>−</i>${score(record,s===1?2:1)}</span><b aria-hidden="true">›</b></button>`;};

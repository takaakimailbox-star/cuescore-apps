...
(() => {
  ...
  function finishRecord(item,players,events,metrics,scores,goals,winner,extra){
    ...
    const playerData=side=>{const playerIndex=side===1?item.p1:item.p2,m=metrics[side],attempts=m.pockets+m.misses;return {name:players[playerIndex].name,registeredPlayerId:players[playerIndex].id,goal:goals[side],skillLevel:item.discipline.disciplineId==="jpa9"?1+((item.index+side)%9):null,score:scores[side],safety:m.safety,fouls:m.fouls,breaks:m.breaks,maxRun:m.maxRun,completedTurns:m.turns,average:m.turns?Number((scores[side]/m.turns).toFixed(3)):0,share:Math.round(scores[side]/Math.max(1,scores[1]+scores[2])*100),misses:m.misses,pocketCount:m.pockets,shotRate:attempts?Math.round(m.pockets/attempts*100):0};};

...
assert.match(html,/confirm\(`サンプルデータ（10名・120試合・全6種目・詳細記録）を準備します。/);
assert.match(html,/alert\(`サンプルデータを準備しました。/);
assert.match(html,/通常データとサンプルデータは別々に処理されます。/);
assert.doesNotMatch(html,/<strong>Official Demo Data<\/strong>|confirm\(`Official Demo Data|公式デモデータを準備しました|通常データとOfficial Demo Dataは別々に処理されます。/);

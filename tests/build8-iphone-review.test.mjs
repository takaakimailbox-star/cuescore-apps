...
test("discipline-fixed history hides selectors, exposes only Detail and restores one level",()=>{assert.match(html,/\.pd8-discipline-fixed \.player-history-disciplines-v4\{display:none!important\}/);assert.match(playerDetail,/openPlayerMatchHistoryV2\?\.\(state\.playerId,state\.discipline\)/);assert.match(playerDetail,/historyRoot\.classList\.add\("hidden"\)/);assert.match(playerDetail,/playerStatsOverlay"\)\?\.classList\.remove\("hidden"\)/);assert.match(playerDetail,/detail\.textContent="詳細"/);assert.match(playerDetail,/analysis\.hidden=true/);assert.match(playerDetail,/analysis\.setAttribute\("aria-hidden","true"\)/);});
test("Match Analysis returns to its Player and discipline fixed history",()=>{assert.match(html,/openMatchAnalysisForPlayerV5\?\.\(button\.dataset\.playerAnalysisRecordId,historyState\.playerId,historyState\.discipline\)/);assert.match(html,/const returnToPlayerHistory=\(\)=>\{document\.getElementById\("rankingsBackBtn"\)\?\.click\(\);setTimeout\(\(\)=>window\.openPlayerMatchHistoryV2\?\.\(selectedViewerPlayerId\|\|selectedPlayerId,selectedOriginDiscipline\),0\)\}/);assert.match(html,/analysisOrigin==="player-history"\)\{returnToPlayerHistory\(\);return\}/);});
...
test("edge Back gesture code and motion state are absent",()=>{assert.doesNotMatch(html,/CueScoreEdgeBack|cue-edge-back|--cue-edge-back-x|cancelInteractiveBack/);});

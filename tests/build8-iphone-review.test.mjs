...
test("discipline-fixed history hides selectors, exposes only Detail and returns one level",()=>{assert.match(html,/\.pd8-discipline-fixed \.player-history-disciplines-v4\{display:none!important\}/);assert.match(playerDetail,/openPlayerMatchHistoryV2\?\.\(state\.playerId,state\.discipline\)/);assert.match(playerDetail,/render\(historyRoot\.dataset\.pd8PlayerId,"detail",historyRoot\.dataset\.pd8Discipline\)/);assert.match(playerDetail,/detail\.textContent="詳細"/);assert.match(playerDetail,/analysis\.hidden=true/);assert.match(playerDetail,/analysis\.setAttribute\("aria-hidden","true"\)/);});

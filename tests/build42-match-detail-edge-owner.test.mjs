...
test("visible Match Detail owns edge Back ahead of underlying Player Back",()=>{
  assert.match(html,/const candidateBackButton = \(\) => \{\s*const matchDetailBack=document\.getElementById\("recordDetailBackBtn"\);\s*if\(document\.body\.classList\.contains\("match-detail-visible-v1"\)&&matchDetailBack\)return matchDetailBack;/);

(() => {
  ...
  function chart(values,key,records=[]){
    ...
    const w=340,h=208,left=48,right=14,top=16,bottom=44,max=percentKeys.has(key)?100:Math.max(1,...valid),min=percentKeys.has(key)?0:Math.min(0,...valid),range=Math.max(1,max-min),step=values.length>1?(w-left-right)/(values.length-1):0;
    const xAt=index=>values.length===1?left+(w-left-right)/2:left+index*step;
    const point=(value,index)=>`${xAt(index)},${h-bottom-((value-min)/range)*(h-top-bottom)}`;
    ...
    const labelEvery=Math.max(1,Math.ceil(values.length/5));
    const showXLabel=index=>index===0||index===values.length-1||index%labelEvery===0;
    ...
    return`<div class="analysis-b4-chart-wrap"><svg class="analysis-b4-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(labels[key])}の推移">${ticks.map(tick=>{const y=h-bottom-((tick-min)/range)*(h-top-bottom);return `<line class="analysis-b4-grid" x1="${left}" y1="${y}" x2="${w-right}" y2="${y}"/><text class="analysis-b4-y-label" x="${left-8}" y="${y+4}" text-anchor="end">${percentKeys.has(key)?`${Math.round(tick)}%`:Math.round(tick*100)/100}</text>`;}).join("")}<line class="analysis-b4-axis" x1="${left}" y1="${top}" x2="${left}" y2="${h-bottom}"/><line class="analysis-b4-axis" x1="${left}" y1="${h-bottom}" x2="${w-right}" y2="${h-bottom}"/>${paths.map(points=>`<polyline class="analysis-b4-plot-line" points="${points.join(" ")}" fill="none"/>`).join("")}${values.map((value,index)=>Number.isFinite(value)?`<circle class="analysis-b4-plot-point" data-b4-point tabindex="0" role="button" aria-label="${esc(xLabels[index]||`試合${index+1}`)} ${valueText(value)}" data-b4-date="${esc(xLabels[index]||`試合${index+1}`)}" data-b4-value="${valueText(value)}" cx="${xAt(index)}" cy="${h-bottom-((value-min)/range)*(h-top-bottom)}" r="5"/>`:"").join("")}${values.map((value,index)=>Number.isFinite(value)&&showXLabel(index)?`<text class="analysis-b4-x-label" x="${xAt(index)}" y="${h-10}" text-anchor="middle">${esc(xLabels[index]||String(index+1))}</text>`:"").join("")}<text class="analysis-b4-axis-caption" x="${w-right}" y="${h-1}" text-anchor="end">試合日</text></svg><output class="analysis-b4-point-callout" data-b4-point-callout hidden></output></div>`;
  }

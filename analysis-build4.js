(() => {
  ...
  const recordDate=record=>{const d=new Date(record?.endedAt||record?.playedAt||record?.startedAt||0);return Number.isNaN(d.getTime())?"日付なし":`${d.getMonth()+1}/${d.getDate()}`;};
  ...
  function chart(values,key,records){
    ...
    const finite=values.filter(Number.isFinite),percent=percentKeys.has(key),rawMin=Math.min(...finite),rawMax=Math.max(...finite),min=percent?0:Math.min(0,rawMin),max=percent?100:Math.max(1,rawMax),range=Math.max(1,max-min),w=620,h=190,left=48,right=48,top=16,bottom=38;
    const x=index=>values.length===1?w/2:left+index*(w-left-right)/(values.length-1),y=value=>h-bottom-((value-min)/range)*(h-top-bottom);
    const ticks=percent?[100,75,50,25,0]:[max,(max+min)/2,min],rawDates=records.map(recordDate),totals=rawDates.reduce((map,date)=>(map[date]=(map[date]||0)+1,map),{}),seen={},dates=rawDates.map(date=>{seen[date]=(seen[date]||0)+1;return totals[date]>1?`${date}${["①","②","③","④","⑤"][seen[date]-1]||`(${seen[date]})`}`:date;}),labelEvery=Math.max(1,Math.ceil(values.length/5));
    ...
    const valueText=value=>fmt(key,value);
    return `<div class="analysis-b4-chart-wrap"><svg class="analysis-b4-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(labels[key])}の推移グラフ">${ticks.map(tick=>`<line class="analysis-b4-grid" x1="${left}" y1="${y(tick)}" x2="${w-right}" y2="${y(tick)}"/><text class="analysis-b4-y-label" x="${left-8}" y="${y(tick)+4}" text-anchor="end">${percent?`${Math.round(tick)}%`:Math.round(tick*100)/100}</text>`).join("")}<line class="analysis-b4-axis" x1="${left}" y1="${top}" x2="${left}" y2="${h-bottom}"/><line class="analysis-b4-axis" x1="${left}" y1="${h-bottom}" x2="${w-right}" y2="${h-bottom}"/>${paths.map(points=>`<polyline class="analysis-b4-plot-line" points="${points.join(" ")}" fill="none"/>`).join("")}${values.map((value,index)=>Number.isFinite(value)?`<circle class="analysis-b4-plot-point" data-b4-point tabindex="0" role="button" aria-label="${esc(dates[index])} ${valueText(value)}" data-b4-date="${esc(dates[index])}" data-b4-value="${valueText(value)}" cx="${x(index)}" cy="${y(value)}" r="5"/>`:"").join("")}${values.map((value,index)=>Number.isFinite(value)&&(index===0||index===values.length-1||index%labelEvery===0)?`<text class="analysis-b4-x-label" x="${x(index)}" y="${h-10}" text-anchor="middle">${esc(dates[index])}</text>`:"").join("")}</svg><output class="analysis-b4-point-callout" data-b4-point-callout hidden></output></div>`;
  }

...
CSVとクラウド同期には、v1.0文書と現行実装・メタデータの差分がある。今回は仕様や機能を変更せず、提出ビルド前の要確認事項として残した。
...
v1.0公式文書はCSV入出力をLaterとしているが、現行 `index.html` にはCSVエクスポートUIと処理が存在する。v1.0提出ビルドで非提供にする方法を決定し、表示・利用できないことを確認する必要がある。

### クラウド同期

v1.0公式文書は自動クラウド同期をLaterとし、データは原則端末内保存としている。一方、現行 `index.html` にはクラウド接続、保存、復元、自動同期設定のコードが存在し、`manifest.webmanifest` は「クラウド同期を利用できます」と記載する。提出ビルドで非提供にする範囲を確定し、Privacy PolicyおよびReview Notesと一致させる必要がある。

### Official Demo Data

Release Scopeに含まれるが、Review Notesでは提出ビルドに残るかを提出前確認としている。収録可否を確定し、実画面名に合わせてReview Notesを更新する必要がある。

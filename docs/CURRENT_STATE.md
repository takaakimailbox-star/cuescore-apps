...
- Build 79 source commitは`a66293a8082498ae28f0d8a96d80d302cca91639`。Bundle ID `com.takaakimailboxstar.cuescoreapps`、Version `1.0`、Build `79`。Archive UUID `080FD5E7-F806-3676-9AFD-500ED91C421B`、executable SHA-256 `a109268c93ecf132d3e50c72ebfd729fe4f584611c206012ed76214d1453fa2b`、`.storekit` 0件。
- Dedicated Dead `16/16`、JPA／Undo／中断復元／History／Backup focused `59/59`、全Node `459/459`、Release Simulator Build、device Archive、IPA export、`git diff --check`はPASS。製品test FAILは0件。
- Appleの公開後version ruleにより、公開済みVersion 1.0へ新しいBuild 79をUploadできない。指示境界に従いVersion 1.1等へ変更せずSTOPしたため、Build 79のApp Store Connect Build IDは存在せず、Internal TestFlight未配布、Product Owner実機確認未実施。
- App Store Connect read-backではBuild 79は0件。公開Version 1.0は`READY_FOR_SALE`／`READY_FOR_DISTRIBUTION`、選択Build 78は`VALID`／`APP_STORE_ELIGIBLE`、Internal `IN_BETA_TESTING`のまま。公開Build 78、App Review、metadata、IAP、価格、availability、Releaseへ変更0件。
- 次Gateには、より高い`CFBundleShortVersionString`を持つ配布候補（例：Version 1.1）を作成するProduct Owner Decisionが必要。新App Store Version作成やApp Review提出はInternal TestFlight upload自体の前提ではないが、このtaskではVersion変更が明示禁止のため実施していない。Evidence: `docs/implementation/CueScore_Build79_JPA_Dead_Ball_Internal_TestFlight_2026-09-26.md`。
...
- Product OwnerはB-04を`ACCEPTED RISK — TESTFLIGHT-SPECIFIC SANDBOX METADATA ISSUE`へ再分類した。TestFlight Build 77はFresh `Product.displayPrice=$5.99`だがApple購入sheetは`¥980`、同一Product ID／同一製品sourceのXcode direct installは`Product.displayPrice=¥980`。製品source、Product ID、StoreKit authorityは変更せず、`¥980` hard-codeやBuild 78も作成しない。
- Xcode Privacy Reportは空白でwarning 0の明示Evidenceにならないため`NOT VERIFIED`を維持する。一方、Build 77は`VALID`／`APP_STORE_ELIGIBLE`、App Privacy回答は現行実装と整合し、Capacitor／Cordovaの`PrivacyInfo.xcprivacy`はtracking false、analytics／advertising SDKなし、Required Reason API declaration欠落の確認Evidenceもない。Privacy evidence gapはApp Review submissionに対して`NON-BLOCKING`と決定し、根拠なしにapp-level privacy manifestを追加しない。
- このDecisionでFinal gateは`READY FOR PRODUCT OWNER APP REVIEW SUBMISSION`となり、その後上記App Review Submissionで提出を完了した。製品変更、Build 78、External TestFlight、自動Releaseは実施していない。A/B Evidence：`docs/release/evidence/CueScore_v1.0_StoreKit_Price_AB_Diagnostic_2026-09-20.md`。
...
- この時点のGateは`READY FOR PRODUCT OWNER FINAL SUBMISSION REVIEW`だった。その後、Final Price / Privacy Decisionで価格をaccepted risk、Privacy evidence gapをnon-blockingと確定し、上記App Review Submissionで提出を完了した。External TestFlight、自動Release、Build 78、製品source変更は実施していない。詳細：`docs/release/CueScore_v1.0_Final_Release_Readiness_Audit_2026-09-20.md`。
...
- Settings focused `44 pass / 0 fail`、全Node `425 pass / 0 fail / 0 skipped`、Release Simulator build、device ArchiveをPASS。Archiveは`com.takaakimailboxstar.cuescoreapps`／`1.0 (75)`、`.storekit` 0件。固定dependencyとsource／native／Archive asset一致を確認。Build source commit `9cfbf671f7d858602cd72aa015443f112cfae21b`をmainへpush。App Store Connect Build ID `1c0b1ec2-24c1-4e73-a15c-c6eed4b950aa`は`VALID`、`usesNonExemptEncryption=false`、Internal group `CueScore Internal Testers`対象。`READY FOR PRODUCT OWNER BUILD 75 SETTINGS SPACING POLISH TEST`でSTOPした。詳細：`docs/implementation/CueScore_Build75_Settings_Spacing_Polish_2026-09-19.md`。
...
- 390×844 fixed tab bar込みでPrivacy bottom `586.5px` <= nav top `776px`、余裕`189.5px`、横overflowなし、initial scrollTop 0を確認。Settings focused `25 pass / 0 fail`、全Node `422 pass / 0 fail / 0 skipped`、Release Simulator build、device ArchiveをPASS。Archiveは`com.takaakimailboxstar.cuescoreapps`／`1.0 (74)`、`.storekit` 0件。Build source commit `416bb4c6a0ba0f43d51ac8ba444edec894d34999`をmainへpush。App Store Connect Build ID `602c7f3f-29bc-461a-ab29-e4f84337fe73`は`VALID`、`usesNonExemptEncryption=false`、Internal group `CueScore Internal Testers`対象。`READY FOR PRODUCT OWNER BUILD 74 SETTINGS REAL-DEVICE FIT TEST`でSTOPした。詳細：`docs/implementation/CueScore_Build74_Settings_Real_Device_Fit_Fix_2026-09-19.md`。
...
- Release Simulator buildとdevice ArchiveをPASS。Archiveは`com.takaakimailboxstar.cuescoreapps`／`1.0 (73)`、`.storekit` 0件。repositoryとArchive checkoutは`ion-ios-filesystem 1.1.2`／revision `0d81e26e828ff9582807e2339112cedf2e0fab85`、`capacitor-swift-pm 8.0.2`で一致。Build source commit `50db6291a12222f31e708c86ce3f68af3123edd3`をmainへpush。App Store Connect Build ID `c494bb28-686a-4dbd-a0cb-60555a4b90db`は`VALID`、`usesNonExemptEncryption=false`、Internal group `CueScore Internal Testers`対象。`READY FOR PRODUCT OWNER BUILD 73 COMPACT SETTINGS TEST`でSTOPした。詳細：`docs/implementation/CueScore_Build73_Compact_Settings_Finalization_2026-09-18.md`。
...
- Pre-Archive GateでXcodeの1.1.4自動切替を検出して正式artifactから除外。`-skipPackageUpdates`を追加した固定手順で再Buildし、正式Archive build graphはrepositoryと同じ`ion-ios-filesystem 1.1.2`／revision `0d81e26e828ff9582807e2339112cedf2e0fab85`を使用。Build source commit `59e95be5342e8e0e7e11cc5d13593ba2a14db083`をmainへpush。App Store Connect Build ID `c0a3dddc-d3f0-4425-a2ab-63ed1b0c249f`は`VALID`、`usesNonExemptEncryption=false`、Internal group `CueScore Internal Testers`対象。詳細：`docs/implementation/CueScore_Build72_Current_Plan_Settings_UI_2026-09-18.md`。

...
- Home中断カード方式を維持し、大型の縦カードからコンパクトな横長カードへ変更した。競技名、Player 1／2、競技別のRace／目標点／持ち点／SL、開始時刻を3段で表示する。
...
- コード、自動テストおよび390×844pxブラウザで、9-Ball再起動後のコンパクトHomeカード、長いPlayer名、Race 2-3、カード全体からの再開、3分岐、横スクロールなしを確認済み。新規試合時の3分岐と保存schema・復元ロジックは変更していない。実iPhone Home Screen PWAでの再確認は未完了。
